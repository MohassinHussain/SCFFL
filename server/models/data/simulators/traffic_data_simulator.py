...
# Try-except block to handle running as script vs module
try:
    ...
    try:
        ...
        # Fallback if file not found (shouldn't happen if setup correctly)
        class MockFuzzy:
...
try:
    ...
    # Group Vehicles by Service Area for Logic
    vehicles_by_area = {}
    for v in vehicles:
...
def generate_traffic_data(num_rows=40000): # Increased to 40k as requested
    ...
    while current_time <= end_date:
        ...
        for route in routes_config:
            ...
            # Vehicle Assignment Logic: Prefer vehicles from this Service Area
            # If not enough, pick random (realistic shuffle)
            ...
            # --- NEW: refined Distribution Cost Calculation ---
            # Constants for Calculation (Rupees) - Tuned for Target Range 50-350
            # C11: Operation Cost (Fixed per trip - Driver fixed portion, Loading etc)
            c_operation_fixed = 40.0 
            
            # C22: Running Cost per minute (Fuel, Refrigeration, Maintenance)
            # Fuel Cost Calculation:
            # Diesel approx 90 Rs/L. 
            # Consumption L/min = (L/100km / 100) * (current speed km/h / 60)
            fuel_price = 90.0
            consumption_per_min = (fuel_eff / 100.0) * (current_speed / 60.0)
            fuel_cost_min = consumption_per_min * fuel_price
            
            # Maintenance & Refrig approx 1.0 Rs/min
            ...
            # C2: Damage Cost (FAP - Fresh Agri Product)
            # Risk increases with Time (perishable) and Traffic Index (bumps/braking)
            # Product Value approx 1000 Rs per crate
            damage_probability = (current_travel_time_min / 600) + (traffic_index / 800)
            
            # Vehicle Condition Impact on Damage (Lower cooling eff -> higher risk)
            if cooling_eff < 90:
            ...
            # C3: Penalty Cost (Late Delivery)
            # Promised time is generous (1.3x free flow). Penalty kicks in after that.
            ...
            # --- NEW: Customer Satisfaction (Fuzzy Logic) ---
            # 1. Time Satisfaction Input: time_deviation (Negative=Early, Positive=Late)
            # 2. Quality Satisfaction Input: Base 100, reduced by damage/weather/traffic
            ...
            # Perishability Impact: Exponential decay of quality after 30 mins
